...
const ATTACH_ML_LINKS = (import.meta.env.VITE_ATTACH_ML_LINKS || 'true') === 'true'
...
const mockMLProducts = [
  { id: 'smartphone-samsung', title: 'Smartphone Samsung Galaxy', searchTerm: 'smartphone samsung galaxy' },
  { id: 'laptop-hp', title: 'Laptop HP Pavilion', searchTerm: 'laptop hp pavilion' },
  { id: 'audifonos-sony', title: 'Audífonos Sony WH-1000XM4', searchTerm: 'audifonos sony wireless' },
  { id: 'monitor-lg', title: 'Monitor LG UltraWide', searchTerm: 'monitor lg ultrawide' },
  { id: 'teclado-logitech', title: 'Teclado Mecánico Logitech', searchTerm: 'teclado mecanico logitech' },
  { id: 'mouse-razer', title: 'Mouse Inalámbrico Razer', searchTerm: 'mouse inalambrico razer' },
  { id: 'camara-canon', title: 'Cámara Canon EOS', searchTerm: 'camara canon eos' },
  { id: 'tablet-ipad', title: 'Tablet iPad Pro', searchTerm: 'tablet ipad pro' },
  { id: 'lampara-mesa', title: 'Lámpara de Mesa LED', searchTerm: 'lampara mesa led' },
  { id: 'audifono-bluetooth', title: 'Audífonos Bluetooth', searchTerm: 'audifonos bluetooth' }
]

function findMockMLProduct(title) {
  const searchTerms = sanitize(title).toLowerCase().split(' ').filter(w => w.length > 2)
  ...
  if (best) {
    // Usar el título original como base para la búsqueda, pero limpio
    const cleanTitle = sanitize(title).replace(/\s+/g, '-').toLowerCase()
    const searchQuery = encodeURIComponent(best.product.searchTerm)
    return {
      source: 'Mercado Libre',
      id: best.product.id,
...
async function findMLForTitle(title, signal){
  log('findMLForTitle called', { title, DISABLE_ML, ATTACH_ML_LINKS, USE_MOCK_ML })
  ...
  log('findMLForTitle searching real ML API', { original: title, sanitized: q, sites: SITE_LIST })
  for (const site of SITE_LIST) {
    try {
      const r = await searchML(site, q, 1, signal)
      log('findMLForTitle ML response', { site, query: q, results: r?.length || 0 })
      if (r && r.length) {
        _mlCache.set(key, r[0])
        log('findMLForTitle found real match', { title, mlId: r[0].id, mlTitle: r[0].title })
        return r[0]
      }
    } catch (e) { 
...
async function attachMlLinks(items, signal){
  ...
  log('attachMlLinks processing', { limited: limited.length, rest: rest.length, limit: ATTACH_ML_LIMIT })

  for (const it of limited) {
    let copy = Object.assign({}, it)
    log('attachMlLinks processing item', { title: it.title, source: it.source })
    try {
      ...
      if (ml) {
        ...
        copy.mlMatched = true
        log('attached ML link SUCCESS', { originalTitle: it.title, mlId: ml.id, mlTitle: ml.title })
      } else {
  ...
  log('attachMlLinks completed', { totalOut: out.length, withMLMatched: out.filter(i => i.mlMatched).length })
  return out
