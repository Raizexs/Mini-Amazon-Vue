  ...
  <section class="mt-4">
    ...
    <div v-else>
      ...
      <div v-else class="row g-2">
        <div class="col-12 col-md-6 col-lg-4" v-for="it in items" :key="it.source + '-' + it.id">
          <a class="card h-100 text-decoration-none" :href="it.permalink" target="_blank" rel="noopener">
            <div class="row g-0 align-items-center">
              <div class="col-3 p-2">
                <img :src="it.thumbnail || '/img/placeholder.png'" class="img-fluid rounded"
                     :alt="it.title" @error="e => e.target.src='/img/placeholder.png'">
              </div>
              <div class="col-9 p-2">
...
async function load(){
  ...
  loading.value = true
  try {
    items.value = await searchExternalOffers(qList.value, { limit: props.limit, randomize: true })
  } catch (e) {
...
onMounted(load)
watch(qList, () => { clearTimeout(t); t = setTimeout(load, props.debounceMs) })
</script>
