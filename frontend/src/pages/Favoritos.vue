<template>
  <main class="container py-4" v-cloak>
    ...
    <div v-if="favCount===0" class="alert alert-info">
      Aún no tienes favoritos.
      <RouterLink class="alert-link" to="/catalogo">Ir al catálogo</RouterLink>.
    </div>

    <div v-else class="row g-3">
      <div class="col-6 col-md-4 col-lg-3" v-for="p in favProducts" :key="p.id">
        <div class="card h-100 product-card">
          <RouterLink :to="`/producto/${encodeURIComponent(p.id)}`" class="text-decoration-none">
            <img :src="imgOf(p)" @error="onImgErr"
                 class="card-img-top" :alt="p.titulo || p.name"
          ...
          <div class="card-body d-flex flex-column">
            ...
            <h2 class="h6 mb-2">
              <RouterLink :to="`/producto/${encodeURIComponent(p.id)}`"
                          class="link-body-emphasis text-decoration-none">
                {{ p.titulo || p.name }}
              </RouterLink>
            </h2>

            <div class="fw-semibold mb-2">{{ fmtCLP(p.precio ?? p.price ?? 0) }}</div>

            <div class="mb-3">
              <span v-if="(p.stock||0) > 0" class="badge text-bg-success">En stock</span>
...
function loadFavs() {
  try { favIds.value = new Set(JSON.parse(localStorage.getItem(FAV_KEY) || '[]')) }
  catch { favIds.value = new Set() }
}
function saveFavs() {
...
function addToCart(p, q = 1) {
  ...
  if (i >= 0) cart[i].qty += q
  else cart.push({ id: p.id, qty: q, precio: p.precio ?? p.price ?? 0, titulo: p.titulo || p.name })
