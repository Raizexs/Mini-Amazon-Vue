<template>
  <div class="row g-4" v-cloak>
            ...
            <div class="accordion-body">
              ...
              >
                <input
                  ...
                  v-model="pricePreset"
                />
                <label class="form-check-label" :for="'price-' + p.id">{{
                  p.label
                }}</label>
              </div>
            </div>
    ...
    >
      <div
        class="d-flex flex-wrap gap-2 align-items-center justify-content-between mb-2"
      >
          ...
          <div class="card h-100 product-card position-relative">
            ...
            <div class="card-actions">
              <button
                ...
                :class="{ 'text-danger': isFav(p.id) }"
                @click="toggleFav(p.id)"
                title="Favorito"
              >
                <i
                  :class="isFav(p.id) ? 'bi bi-heart-fill' : 'bi bi-heart'"
                ></i>
              </button>
              <button
                class="card-action btn"
            ...
            <div class="card-body d-flex flex-column">
              ...
              <div class="d-flex align-items-center gap-2 mb-2">
                ...
                <span class="badge text-bg-warning" v-if="(p.rating || 0) > 0"
                  >★ {{ (p.rating || 0).toFixed(1) }}</span
                >
              </div>
              <div class="mb-2">
                <span v-if="p.stock > 0" class="badge text-bg-success"
                  ><i class="bi bi-check2-circle me-1"></i> En stock</span
                >
                <span v-else class="badge text-bg-secondary"
                  ><i class="bi bi-slash-circle me-1"></i> Sin stock</span
                >
              </div>
      ...
      <nav class="mt-3" v-if="totalPages > 1">
        <ul class="pagination pagination-sm mb-0">
          <li class="page-item" :class="{ disabled: page <= 1 }">
            <button class="page-link" @click="page = 1" aria-label="Primera">
              &laquo;
            </button>
          </li>
          <li class="page-item" :class="{ disabled: page <= 1 }">
            <button class="page-link" @click="page--" aria-label="Anterior">
              &lsaquo;
            </button>
          </li>
          <li
          ...
          <li class="page-item" :class="{ disabled: page >= totalPages }">
            <button class="page-link" @click="page++" aria-label="Siguiente">
              &rsaquo;
            </button>
          </li>
          <li class="page-item" :class="{ disabled: page >= totalPages }">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content">
        <div class="modal-body p-0">
          <div class="row g-0">
            <div class="col-md-5 p-3 text-center border-end">
              <img
                :src="qv?.images?.[0] || '/img/placeholder.png'"
                class="img-fluid"
            ...
            <div class="col-md-7 p-3">
              ...
              <div class="d-flex align-items-center gap-2 mb-2">
                ...
                <span class="badge text-bg-warning" v-if="(qv?.rating || 0) > 0"
                  >★ {{ (qv?.rating || 0).toFixed(1) }}</span
                >
              </div>
              ...
              <div class="mb-3">
                <span v-if="(qv?.stock || 0) > 0" class="badge text-bg-success"
                  >En stock</span
                >
              ...
              <div class="d-flex gap-2">
                ...
                <button
                  ...
                  :disabled="(qv?.stock || 0) === 0"
                  @click="addToCart(qv, 1)"
                >
        ...
        <div class="modal-footer py-2">
          <button class="btn btn-primary w-20" @click="gotoProduct(qv)">
            Ver producto
          </button>
...
const qvSpecs = computed(() => {
  ...
  extras.push({
    k: "Stock",
    v: qv.value?.stock > 0 ? `${qv.value.stock} disp.` : "Sin stock",
  });
...
const priceRange = computed(
  () =>
    PRICE_PRESETS.find((x) => x.id === pricePreset.value) || PRICE_PRESETS[0]
);
...
watch(
  [search, cat, pricePreset, brand, rating, onlyStock, sort, pageSize],
  () => {
    page.value = 1;
  }
);
...
function applyFilters() {
  ...
  if (pricePreset.value && pricePreset.value !== "all")
    q.price = pricePreset.value;
...
onMounted(async () => {
  ...
  brands.value = [
    ...new Set(all.value.map((p) => p.brand).filter(Boolean)),
  ].sort();
  ...
  if (typeof q.rating === "string" || typeof q.rating === "number")
    rating.value = Number(q.rating);
...
/* Allow text selection only on search input */
.form-control {
  cursor: text;
  user-select: text;
...
/* Checkboxes and labels should not allow text selection */
.form-check-label {
  user-select: none;
  -webkit-user-select: none;
  cursor: pointer;
