      ...
      <section class="col-12 col-lg-8">
        <div class="card cart-box">
          <div class="card-body">
              ...
              <table class="table align-middle mb-0">
                ...
                <tbody>
                  <tr v-for="ln in lines" :key="ln.p.id">
                    <td>
                      <img :src="imgOf(ln.p)" @error="onImgErr"
                           class="rounded" width="48" height="48" style="object-fit:cover;">
                    </td>
                    <td>
                      <div class="fw-semibold">
                        <RouterLink :to="`/producto/${encodeURIComponent(ln.p.id)}`" class="link-body-emphasis">
                          {{ ln.p.titulo || ln.p.name }}
                        </RouterLink>
                      </div>
                      <div class="small text-muted">
                        {{ ln.p.marca || ln.p.brand }} • {{ ln.p.categoria || ln.p.catName }}
                      </div>
                    </td>
                    <td class="text-end">{{ fmtCLP(priceOf(ln.p, ln.price)) }}</td>
                    <td>
                      <div class="input-group input-group-sm" style="max-width:170px;">
                        <button class="btn btn-outline-secondary" @click="decQty(ln)" :disabled="ln.qty<=1">−</button>
                        <input type="number" class="form-control text-center"
                               min="1" :max="stockOf(ln.p)" v-model.number="ln.qty" @change="clampQty(ln)">
                        <button class="btn btn-outline-secondary" @click="incQty(ln)" :disabled="ln.qty>=stockOf(ln.p)">+</button>
                      </div>
                      <div class="form-text text-danger" v-if="ln.qty>stockOf(ln.p)">
                    ...
                    <td class="text-end">
                      <button class="btn btn-sm btn-outline-danger" @click="askRemove(ln)">Quitar</button>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <button class="btn btn-link text-danger mt-3" @click="clearCart">Vaciar carrito</button>
          </div>
        </div>
      </section>
        ...
        <div class="cart-summary">
          <div class="card">
            <div class="card-body">
              ...
              <RouterLink class="btn btn-primary w-100 mt-3" to="/checkout">Ir al checkout</RouterLink>
            </div>
          </div>
        </div>
      </aside>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ¿Seguro que quieres quitar <strong>{{ pending?.p?.titulo || pending?.p?.name || 'este producto' }}</strong> del carrito?
        </div>
...
const imgOf = p => (p?.imagenes?.[0] || p?.images?.[0] || '/img/placeholder.png')
...
async function loadCart () {
  ...
  lines.value = raw.map(it => {
    ...
    return {
      p: p || { id: it.id, titulo: it.titulo || 'Producto', precio: it.precio || it.price || 0, imagenes: [] },
      qty: Number(it.qty || 1),
...
function decQty (ln) { if (ln.qty > 1) { ln.qty--; save() } }
function incQty (ln) { if (ln.qty < stockOf(ln.p)) { ln.qty++; save() } }
