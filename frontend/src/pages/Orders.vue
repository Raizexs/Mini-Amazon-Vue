        ...
        <div class="card-body">
          ...
          >
            ...
            <div class="d-flex align-items-center gap-3">
              <span class="badge rounded-pill" :class="statusClass(o.status)">{{
                statusText(o.status)
              }}</span>
          ...
          <div v-if="openIdx === idx" class="mt-3 vstack gap-4">
                ...
                >
                  ...
                  <div class="flex-grow-1">
                    <div class="small fw-semibold">
                      <RouterLink
                        :to="`/producto/${encodeURIComponent(it.id)}`"
            ...
            <section>
              ...
              <div class="d-flex justify-content-between">
                <span>Subtotal</span
                ><strong>{{ fmtCLP(o.totals.subtotal) }}</strong>
              </div>
              <div class="d-flex justify-content-between">
                <span>Descuento</span>
              ...
              <div class="d-flex justify-content-between">
                <span>Envío</span>
                <strong>{{
                  o.totals.shipping ? fmtCLP(o.totals.shipping) : "Gratis"
                }}</strong>
              </div>
              <div class="d-flex justify-content-between">
            ...
            <section>
              ...
              <template v-else>
                <div class="small fw-semibold">
                  ...
                  <span class="text-muted">
                    ({{ o.shipping.method?.dias || "—" }})</span
                  >
                </div>
                <div
                  class="text-muted small order-address"
                  v-if="o.shipping.address"
                >
                  {{ o.shipping.address.address }},
                  {{ o.shipping.address.city }}, {{ o.shipping.address.region }}
                  <template v-if="o.shipping.address.depto"
                    >, {{ o.shipping.address.depto }}</template
                  >
                  <template v-if="o.shipping.address.notes">
                    — {{ o.shipping.address.notes }}</template
                  >
                </div>
              </template>
              ...
              <div class="mt-2" v-if="o.coupon">
                <span class="badge bg-success"
                  >Cupón: {{ o.coupon.code || o.coupon }}</span
                >
              </div>
...
const ordersFiltered = computed(() =>
  filterStatus.value
    ? ordersSorted.value.filter((o) => o.status === filterStatus.value)
    : ordersSorted.value
);
const selected = computed(
  () =>
    orders.value.find((o) => String(o.id) === String(selectedId.value)) || null
);
const listForRender = computed(() =>
  isDetailMode.value && selected.value ? [selected.value] : ordersFiltered.value
);
...
watch([listForRender, isDetailMode], () => {
  if (isDetailMode.value) {
    ...
    const i = ordersFiltered.value.findIndex(
...
function recomprar(o) {
  ...
  for (const it of o.items) {
    ...
    if (idx >= 0) cart[idx].qty += it.qty;
      ...
      cart.push({ id: it.id, qty: it.qty, precio: it.price, titulo: it.name });
  }
  localStorage.setItem(key, JSON.stringify(cart));
  toast("Productos agregados al carrito 🛒");
}
function clearHistory() {
  if (!confirm("¿Seguro que quieres borrar todo el historial de pedidos?"))
    return;
...
function payText(p) {
  if (!p) return "—";
  if (p.method?.startsWith?.("express:"))
    return `Pago rápido (${p.method.split(":")[1]})`;
  if (p.method === "card")
    return `Tarjeta ${p.last4 ? "••••" + p.last4 : ""}`.trim();
