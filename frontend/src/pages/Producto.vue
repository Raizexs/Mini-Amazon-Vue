<template>
  ...
  <main class="container py-4">
    ...
    <div v-else class="row g-4">
      ...
      <div class="col-12 col-md-6">
        <div class="card">
          <div class="ratio ratio-1x1">
            <img :src="imagenes[imgIndex]" class="img-fluid object-fit-contain p-3"
                 :alt="producto.titulo" @error="onImgError" />
          </div>
          <div class="d-flex gap-2 p-3 flex-wrap">
            <button v-for="(src,i) in imagenes" :key="i"
                    class="btn btn-outline-secondary p-0"
                    :class="{'border-2': i===imgIndex}"
                    style="width:70px;height:70px"
                    @click="imgIndex=i">
              <img :src="src" class="img-fluid object-fit-cover w-100 h-100 rounded" @error="onImgError" />
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-md-6">
        ...
        <div class="d-flex align-items-center gap-2 mb-2">
          ...
          <span v-if="producto.stock===0" class="badge text-bg-secondary">Sin stock</span>
        ...
        <div class="d-flex align-items-center gap-2 my-3">
          <button class="btn btn-outline-secondary" @click="decQty" :disabled="qty<=1">−</button>
          <input class="form-control text-center" style="width:80px" type="number" min="1"
                 :max="producto.stock || 1" v-model.number="qty">
          <button class="btn btn-outline-secondary" @click="incQty" :disabled="qty>=producto.stock">+</button>
        </div>

        <div class="d-flex gap-2">
          <button class="btn btn-primary" :disabled="producto.stock===0" @click="addToCart">
            <i class="bi bi-bag-plus me-1"></i> Agregar al carrito
          </button>
          <button class="btn btn-outline-danger" @click="toggleFav(producto.id)">
            <i class="bi" :class="isFav(producto.id)?'bi-heart-fill':'bi-heart'"></i>
        ...
        <div class="mt-3">
          ...
          <span class="ms-2">
            <i v-for="i in 5" :key="i" class="bi"
               :class="(i<=Math.round(ratingProm)) ? 'bi-star-fill' : 'bi-star'"></i>
    ...
    <!-- Reviews -->
    <section v-if="producto" class="mt-5">
      <h3 class="h5 mb-3">Opiniones</h3>

      <div v-if="reviews.length===0" class="text-muted">Aún no hay reseñas.</div>
      <div v-else class="vstack gap-3">
        <div v-for="r in reviews" :key="r.id" class="border rounded p-3">
          <div class="d-flex align-items-center justify-content-between">
            <strong>{{ r.autor }}</strong>
            <div>
              <i v-for="i in 5" :key="i" class="bi" :class="(i<=r.rating)?'bi-star-fill':'bi-star'"></i>
            </div>
          </div>
      ...
      <form class="mt-4" @submit.prevent="submitReview">
        <div class="row g-2 align-items-center">
          <div class="col-sm-4">
            <input class="form-control" v-model.trim="formReview.autor" placeholder="Tu nombre" required>
          </div>
          <div class="col-sm-2">
            <select class="form-select" v-model.number="formReview.rating" required>
          ...
          <div class="col-sm-6">
            <input class="form-control" v-model.trim="formReview.comentario" placeholder="Escribe tu opinión" required>
          </div>
        </div>
        <button class="btn btn-outline-primary mt-3" type="submit">Publicar reseña</button>
      </form>
    </section>
    ...
    <!-- Relacionados -->
    <section v-if="relacionados.length" class="mt-5">
      <h3 class="h5 mb-3">También te puede interesar</h3>
      <div class="row g-3">
        <div class="col-6 col-md-4 col-lg-3" v-for="p in relacionados" :key="p.id">
          <div class="card h-100">
            <img :src="imgUrl(p) || '/img/placeholder.png'" class="card-img-top" :alt="p.titulo" @error="onImgError" />
            <div class="card-body">
            ...
            <div class="card-footer bg-transparent">
              <RouterLink class="btn btn-sm btn-outline-primary w-100" :to="`/producto/${encodeURIComponent(p.id)}`">
                Ver producto
              </RouterLink>
            </div>
          </div>
...
function fmtDate(d) {
  ...
  if (typeof d === 'number') {
    ...
    return isNaN(dt) ? '' : dt.toLocaleDateString('es-CL', { year: 'numeric', month: 'short', day: '2-digit' })
  }
  const dt = new Date(String(d))
  return isNaN(dt) ? '' : dt.toLocaleDateString('es-CL', { year: 'numeric', month: 'short', day: '2-digit' })
}

const onImgError = e => { e.target.src = '/img/placeholder.png' }
...
function pickRelated(all, current, min = 3, max = 8) {
  ...
  // 1) Misma categoría
  const sameCat = all
    .filter(p => p.categoria === current.categoria && String(p.id) !== String(current.id))
    .sort((a, b) =>
  ...
  for (const p of sameCat) {
    ...
    out.push(p); seen.add(String(p.id))
  }
...
/* ---------- Favoritos ---------- */
function loadFavs () {
  try { return new Set(JSON.parse(localStorage.getItem('mini.favs') || '[]')) }
  catch { return new Set() }
}
function saveFavs (set) { localStorage.setItem('mini.favs', JSON.stringify([...set])) }
...
function addToCart () {
  ...
  if (i >= 0) cart[i].qty = Math.min((cart[i].qty || 1) + qty.value, producto.value.stock || 1e9)
  else cart.push({ id: producto.value.id, qty: qty.value, precio: producto.value.precio, titulo: producto.value.titulo })
...
const incQty = () => { if (qty.value < (producto.value?.stock || 1)) qty.value++ }
...
function normalizeReview(r) {
  ...
  const rawAutor =
    r.autor ?? r.author ?? r.nombre ?? r.name ??
    r.usuario ?? r.user ?? r.nickname ?? r.nick ??
    r.cliente ?? r.displayName ?? ''
...
function localReviewsKey (pid) { return `mini.reviews.${pid}` }
function loadLocalReviews (pid) {
  try { return JSON.parse(localStorage.getItem(localReviewsKey(pid)) || '[]') }
  catch { return [] }
}
...
function submitReview () {
  ...
  const locals = [normalizeReview(r), ...loadLocalReviews(pid).map(normalizeReview)]
  ...
  const el = document.getElementById('appToast')
  if (el && window.bootstrap?.Toast) {
    el.querySelector('.toast-body')?.replaceChildren(document.createTextNode('¡Gracias por tu reseña! ⭐'))
    new window.bootstrap.Toast(el).show()
  }
}
...
async function loadData () {
  ...
  const list = Array.isArray(allProducts) ? allProducts : (allProducts?.productos || [])
  const prod = list.find(p => String(p.id) === String(id))

  if (!prod) { cargando.value = false; return }

  producto.value = prod
  imagenes.value = (prod.imagenes && prod.imagenes.length)
  ? prod.imagenes.map(fixSrc)
  : ['/img/placeholder.png']

  // Reviews: archivo + locales (normalizadas)
  const rawSrv = (Array.isArray(allReviews) ? allReviews : (allReviews?.reviews || []))
