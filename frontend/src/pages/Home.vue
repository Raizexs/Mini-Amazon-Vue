      ...
      <div class="col-6 col-md-4 col-lg-3" v-for="p in destacados" :key="p.id">
        <div class="card product-card h-100">
          <div class="position-relative">
            ...
            <span v-if="p.stock===0" class="badge text-bg-secondary position-absolute top-0 start-0 m-2">Sin stock</span>

            <div class="card-actions">
              <button class="card-action" :class="{'fav-active': isFav(p.id)}" @click="toggleFav(p.id)" title="Favorito">
                <i class="bi" :class="isFav(p.id)?'bi-heart-fill':'bi-heart'"></i>
              </button>
              <RouterLink class="card-action" :to="`/producto/${encodeURIComponent(p.id)}`" title="Ver">
                <i class="bi bi-eye"></i>
              </RouterLink>
            </div>
          </div>
          ...
          <div class="card-body">
            ...
            <div class="small text-muted">{{ p.categoria }} • {{ p.marca }}</div>
